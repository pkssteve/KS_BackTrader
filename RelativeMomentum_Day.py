...
if __name__ == "__main__":
    ...
    for i in range(0, len(btc)):
        ...
        if i > len(btc[btc['macd'].isna()]) + 1:
            ...
            mclose2[['Diff']] = (mclose2['Close_x']-mclose2['Close_y'])/ mclose2['Close_y']
    ...
    for key, value in coinprofit.items():
        ...
        listdf[iter]['Profit'] = listdf[iter][key]/unitbuy - 1
        iter += 1
    ...
    for df in listdf:
        if len(df)>0:
            ...
            coin_earning_rate = ((df[cname].sum() - totalcash)/ totalcash) * 100

            print("[%s] Trades:%d, Price: %.4f, win rate:%.1f%%, +:%.1f%%, -:%.1f%%, Profit:%.2f%%" %
                  (cname, observation, close, win_rate, totalPlus, totalMinus, coin_earning_rate))


    vdf['PortValue'].plot()
    print("Init cash: %.2f, Final Value: %.2f, Profit: %.2f" % (init_cash, final_value, ((final_value-init_cash)/init_cash)*100))
