            ...
            if self.RSIState["state"] == "not started":
                ...
                if (self.RSI[0] > self.RSI[-1] and self.RSI[-1] <= rsiBoundry and self.RSI[0] > rsiGapValue):
                    self.save_RSI("RSI momentum", len(self),
                                  self.dataclose[0], self.RSI[0])
                # elif (self.RSI[0] > self.RSI[-1] and self.RSI[-1] <= rsiBoundry and self.RSI[0] > rsiGapValue):
                #     self.save_RSI("RSI momentum", len(self),
                #                   self.dataclose[0], self.RSI[0])
                ...
                if (self.dataclose[0] <= self.buyprice * 0.91) and (len(self) - self.RSIState["len"]) > 2:
                    retval = -1
                    self.save_RSI("not started", len(self),
                                  self.dataclose[0], self.RSI[0])
