...
class RSIMomentum(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.macd = bt.indicators.MACD(self.datas[0], plot=True, movav =ExponentialMovingAverage)
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                profitRate = (curValue  - self.positionValue) / self.positionValue
                ...
                commsMinus = commsMinus + ((buyValue + (buyValue*profitRate)) * self.commission) # when sell
                self.curcash = self.curcash + (buyValue * profitRate) - commsMinus
                if self.leverage > 1:
    ...
    def RSIstatemachine(self):
        ...
        if len(self) != self.RSIState["len"]:
            ...
            if self.RSIState["state"] == "not started":
                if (self.RSI[0] > self.RSI[-1] and self.RSI[-1] <= rsiBoundry):  # and self.RSI[0] > 30
                ...
                if self.RSI[0] < 30 or self.RSI[0] > 65:
                    self.save_RSI("not started", len(self), self.dataclose[0], self.RSI[0])
                elif self.macd.lines.macd[0] >= self.macd.lines.signal[0]:
                    self.save_RSI("MACD cross", len(self), self.dataclose[0], self.RSI[0])
                ...
                #     self.buyprice = self.RSIState["price"]

                if self.dataclose[0] <= self.buyprice * 0.89:
                    retval = -1
                    self.save_RSI("not started", len(self),
                                  self.dataclose[0], self.RSI[0])

                elif self.RSI[0] >= 70 and self.RSI[0] < self.RSI[-1]:
