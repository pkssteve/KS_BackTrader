    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                profitRate = (order.executed.value  - self.positionValue) / self.positionValue
    ...
    def RSIstatemachine(self):
        ...
        if len(self) != self.RSIState["len"]:
            ...
            if self.RSIState["state"] == "not started":
                if (self.RSI[0] > self.RSI[-1] and self.RSI[-1] <= 50):  # and self.RSI[0] > 30
                ...
                if self.dataclose[0] <= self.buyprice * 0.96:
                    retval = -1
                    self.save_RSI("not started", len(self),
                                  self.dataclose[0], self.RSI[0])
                elif self.RSI[0] >= 60:
                ...
                if self.RSI[0] > self.RSI[-1] and self.RSI[0] > 67:
                    retval = 1
