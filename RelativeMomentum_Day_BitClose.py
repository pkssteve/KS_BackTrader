...
if __name__ == "__main__":
    ...
    macd, macd_s, macd_hist = talib.MACDEXT(btc['Close'], fastperiod=3, fastmatype=talib.MA_Type.EMA, slowperiod=9, slowmatype=talib.MA_Type.EMA, signalmatype=talib.MA_Type.EMA, signalperiod = 5)
            ...
            if position == 0:
                if macd.iloc[i] > macd_s.iloc[i]:
                    ...
                    for j in range(0, numStocks):
                        if len(curClose[curClose['Name'] == mclose_p.iloc[j]['Name']]) > 0:
                    ...
                    shortlist = {}
                    for j in range(0, numStocks):
                        if len(curClose[curClose['Name'] == mclose_m.iloc[j]['Name']]) > 0:
