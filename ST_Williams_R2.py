...
class WillR2(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.willR = bt.indicators.WilliamsR(self.datas[0], period = 14)
        self.willR2 = 100 * (self.dataclose - self.pmax) / (self.pmax - self.pmin)
    ...
    def RSIstatemachine(self):
        ...
        if len(self) != self.RSIState["len"]:
            ...
            if self.RSIState["state"] == "not started":
                ...
                if self.willR2[0] <= - 95:
                    self.save_RSI("prebuy", len(self),
                                  self.dataclose[0], self.RSI[0])

            elif self.RSIState["state"] == "prebuy":
                if lenDiff >= 5:
                    if self.willR2[0] > self.willR2[-1] and (self.willR2[0] > -90):
                ...
                elif self.willR2[0] > - 10:
                    self.save_RSI("presell", len(self),
                                  self.dataclose[0], self.RSI[0])

            elif self.RSIState["state"] == "presell":
                if lenDiff >= 5:
                    if self.willR2[0] < self.willR2[-1] and (self.willR2[0] <= -10):
                        retval = -1
                        self.save_RSI("not started", len(self),
                                      self.dataclose[0], self.RSI[0])
